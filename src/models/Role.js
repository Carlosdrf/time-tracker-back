...
roleModel.assignRole = async(user_id, role_id = 1) =>{
...
roleModel.verifyUserRole = async(user_id) =>{
    return await pool.query('SELECT role.name FROM role INNER JOIN user_role ur ON ur.role_id = role.id INNER JOIN users u ON u.id = ur.user_id WHERE u.id = ?', [user_id])
}
