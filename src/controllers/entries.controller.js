...
export const deleteEntry = async (req, res) => {
  ...
  if (entry) {
    if (userRole.role_id == roleModel.USER_ROLE) {
      await db.entries.update({ status: 2 }, { where: { id: req.params.id } });
      return res.status(200).json({ message: "Disabled" });
    }
    if (userRole.role_id == roleModel.ADMIN_ROLE) {
